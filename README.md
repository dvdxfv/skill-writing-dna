...
这个 Skill 的思路不是"写一个更聪明的提示词"，而是分三层来解决：

| 层级 | 做什么 | 怎么做的 |
|:---|:---|:---|
| 🧹 **Layer 1** | 通用模板检测与剥离 | 跨文档对齐——自动发现任何文体中反复出现的模板段落，不依赖文体预设 |
| 🧬 **Layer 2** | 写作风格 DNA 提取 | 从你自己的文章里统计用词习惯、句式节奏、开头结尾模式 |
| ✏️ **Layer 3** | DNA 驱动改写 | 拿着你的 DNA 去改 AI 草稿——清除套话、植入你的表达习惯 |

关键设计：**Layer 1 文体无关**——不预设"什么是公文模板""什么是小红书模板"，而是通过跨文档重复率对比自动发现。无论你写的是公文、技术报告、公众号还是学术论文，都能自动识别并剥离其中的模板内容。

中间设了两个必停的人工确认点——DNA 画像对不对（🔵第4步）、改写效果好不好（🔵第7步）——因为风格这件事，拍板的必须是你。
