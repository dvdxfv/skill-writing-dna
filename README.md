...
```
① 放入样本 → ② 提取 DNA → ③ 放入 AI 草稿 → ④ 按 DNA 改写 → 交付
```

| 步骤 | 做什么 | 输出 |
|:---:|:---|:---|
| 1 | `run.py pipeline` — 自动完成 DOCX转换→非正文过滤→模板剥离 | 干净的正文文本 |
| 2 | `run.py extract` — 自动统计 + 人工复核，产出 DNA 画像 | DNA JSON + 特征云图 |
| 3 | 把 AI 生成的草稿放入 `inputs/ai_drafts/` | — |
| 4 | `run.py rewrite` — 按你的 DNA 规则改写 AI 草稿 | 改写成品稿 |

> 🔵 流程中标记"用户确认"的节点可以跳过或直接接受默认结果。
