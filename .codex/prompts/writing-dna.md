...
1. **信息无损**：原文每个事实点、数字、产品名、结论必须保留。
2. **黑名单硬清除**：DNA 中 `blacklist_phrases` 里的词组一个都不能出现。
3. **签名短语自然植入**：在合适处使用 `signature_phrases`，不要硬塞（每 150-200 字 1 个为宜）。
4. **句长结构对齐**：按 `sentence_features` 调整句长与节奏。
